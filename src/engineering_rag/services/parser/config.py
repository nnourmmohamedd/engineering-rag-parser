...
:mod:`engineering_rag.services.parser.converter` (Docling object construction)
and :mod:`engineering_rag.services.parser.profiles` (profile selection) are
the only places that translate those into the installed Docling types. That
keeps Docling API churn out of user-facing YAML and out of stored run
manifests.
