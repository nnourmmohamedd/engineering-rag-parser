...
This is the application boundary: the one place logging is configured
(:func:`engineering_rag.utils.logging.configure_logging`) and the one place
that calls into orchestration (:mod:`engineering_rag.pipelines.parsing_pipeline`).
...
A future HTTP interface (FastAPI) belongs in this package too, alongside this
module — not inside any service.
"""
...
from engineering_rag.services.parser import ParserConfig, PreflightError, Profile, load_config
from engineering_rag.services.parser.models import RunStatus, Severity, ValidationReport
...
def _force_utf8_streams() -> None:
    ...
    handful of glyphs to `?` rather than crashing the process. Must run before
    `configure_logging` installs a stderr handler, so that handler inherits a
    UTF-8-safe stream too.
    """
...
def _setup_logging(level: str, quiet: bool, log_file: Path | None = None) -> None:
    """Configure the centralized `engineering_rag` logger tree for this command.

    Routes to stderr so `--json` on stdout stays machine-parseable. This is
    the *only* place `configure_logging` is called for CLI use — a pipeline
    run additionally attaches its own per-run file handler once a run
    directory exists (see `ParserService.run`), on top of whatever this sets
    up.
    """
    configure_logging(console_level=level, quiet_console=quiet, log_file=log_file)
...
def inspect_cmd(
    ...
    log_file: Annotated[
        Path | None, typer.Option("--log-file", help="Optional file to also write logs to.")
    ] = None,
) -> None:
    """Run preflight only: an independent source inventory, without Docling."""
    _setup_logging(log_level, quiet=as_json, log_file=log_file)
...
def run_cmd(
    ...
    artifacts: Annotated[
        Path | None, typer.Option("--artifacts", help="Base directory for run artifacts.")
    ] = None,
    ...
    ``--artifacts`` defaults to ``data/output/parser`` when omitted; every
    completed run additionally gets its own ``logs/engrag.log`` under the run
    directory regardless of ``--log-file``.
    """
    _setup_logging(log_level, quiet=as_json, log_file=log_file)
    config = _load(config_path, profile, strict, log_level)
    output_root = artifacts if artifacts is not None else default_parser_output_root()
    try:
        result = run_parsing_pipeline(input_path, config, output_root)
    except PreflightError as exc:
        console.print(f"[bold red]Preflight rejected the input:[/] {exc}")
        raise typer.Exit(2) from exc
...
def validate_cmd(
    ...
    ``--run`` accepts any existing run directory, including one produced
    under the previous ``artifacts/`` root — the report is read from disk,
    not re-derived from a fixed location.
    """
